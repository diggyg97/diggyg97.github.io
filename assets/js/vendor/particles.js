		...
		t && Object.deepExtend(v, t),
			...
			(v.fn.retinaInit = function () {
				v.retina_detect && 1 < window.devicePixelRatio
					? ((v.canvas.pxratio = window.devicePixelRatio),
					  (v.tmp.retina = !0))
					...
					(v.particles.size.value =
						v.tmp.obj.size_value * v.canvas.pxratio),
					(v.particles.size.anim.speed =
						v.tmp.obj.size_anim_speed * v.canvas.pxratio),
					(v.particles.move.speed =
						v.tmp.obj.move_speed * v.canvas.pxratio),
				...
				(this.radius =
					...
					this.y > v.canvas.h - 2 * this.radius
						...
						: this.y < 2 * this.radius &&
						  (this.y = this.y + this.radius),
					v.particles.move.bounce &&
						v.fn.vendors.checkOverlap(this, a),
					(this.color = {}),
						...
						? e.value instanceof Array
							? ((a =
									e.value[
										Math.floor(
											Math.random() *
												v.particles.color.value.length
										)
									]),
				...
				switch (
					...
					case "polygon":
						v.fn.vendors.drawShape(
							v.canvas.ctx,
							i.x -
								e / (v.particles.shape.polygon.nb_sides / 3.5),
							i.y - e / 0.76,
							(2.66 * e) /
								(v.particles.shape.polygon.nb_sides / 3),
					...
					case "star":
						v.fn.vendors.drawShape(
							...
							i.x -
								(2 * e) /
									(v.particles.shape.polygon.nb_sides / 4),
							i.y - e / 1.52,
							(2 * e * 2.66) /
								(v.particles.shape.polygon.nb_sides / 3),
							v.particles.shape.polygon.nb_sides,
				...
				v.canvas.ctx.closePath(),
					0 < v.particles.shape.stroke.width &&
						((v.canvas.ctx.strokeStyle =
							v.particles.shape.stroke.color),
						(v.canvas.ctx.lineWidth =
							v.particles.shape.stroke.width),
						v.canvas.ctx.stroke()),
					v.canvas.ctx.fill();
			}),
			...
			(v.fn.particlesUpdate = function () {
				for (var e = 0; e < v.particles.array.length; e++) {
					...
					if (
						...
						v.particles.opacity.anim.enable &&
							(1 == a.opacity_status
								? (a.opacity >= v.particles.opacity.value &&
								  ...
								  (a.opacity += a.vo))
								: (a.opacity <=
										v.particles.opacity.anim.opacity_min &&
										(a.opacity_status = !0),
								  (a.opacity -= a.vo)),
							a.opacity < 0 && (a.opacity = 0)),
						v.particles.size.anim.enable &&
							(1 == a.size_status
								? (a.radius >= v.particles.size.value &&
										(a.size_status = !1),
								  (a.radius += a.vs))
						...
						isInArray(
							"grab",
							v.interactivity.events.onhover.mode
						...
						(isInArray(
							...
							isInArray(
								"bubble",
								v.interactivity.events.onclick.mode
							)) &&
						...
						for (var i = e + 1; i < v.particles.array.length; i++) {
							...
							v.particles.line_linked.enable &&
								...
								v.particles.move.bounce &&
									v.fn.interact.bounceParticles(a, n);
						}
				}
			}),
				...
				isInArray("bubble", v.interactivity.events.onhover.mode)
					...
					: v.interactivity.events.onclick.enable &&
					  ...
					  (v.tmp.bubble_clicking &&
							...
							(p =
								(new Date().getTime() -
									v.interactivity.mouse.click_time) /
				...
				"mousemove" == v.interactivity.status
					...
					: v.interactivity.events.onclick.enable &&
					  ...
					  v.tmp.repulse_clicking
							? ((i = Math.pow(
									v.interactivity.modes.repulse.distance / 6,
									3
							  )),
									...
									(function () {
										...
										(a.vx = c * Math.cos(t)),
											...
											"bounce" ==
												...
												((t = a.x + a.vx),
												(e = a.y + a.vy),
												(t + a.radius > v.canvas.w ||
													t - a.radius < 0) &&
													(a.vx = -a.vx),
												(e + a.radius > v.canvas.h ||
													e - a.radius < 0) &&
													(a.vy = -a.vy));
									})())
					...
					(a = Math.sqrt(a * a + t * t)) <=
						...
						0 <
							(t =
								...
								a /
									(1 /
										v.interactivity.modes.grab.line_linked
											.opacity) /
									v.interactivity.modes.grab.distance) &&
						((a = v.particles.line_linked.color_rgb_line),
						(v.canvas.ctx.strokeStyle =
			...
			(v.fn.vendors.eventsListeners = function () {
				"window" == v.interactivity.detect_on
					...
					(v.interactivity.events.onhover.enable ||
						v.interactivity.events.onclick.enable) &&
						(v.interactivity.el.addEventListener(
							"mousemove",
							function (e) {
								var t;
								(e =
								...
								if (
									...
									switch (
										...
										case "push":
											v.particles.move.enable ||
											1 ==
												v.interactivity.modes.push
													.particles_nb
												? v.fn.modes.pushParticles(
														v.interactivity.modes
															.push.particles_nb,
				...
				"image" == v.particles.shape.type
					? "svg" == v.tmp.img_type
						? v.tmp.count_svg >= v.particles.number.value
							? (v.fn.particlesDraw(),
							  v.particles.move.enable
									? (v.fn.drawAnimFrame = requestAnimFrame(
											v.fn.vendors.draw
									  ))
